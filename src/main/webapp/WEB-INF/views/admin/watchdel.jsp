								...
								<table class="table table-bordered" id="dataTable" width="100%"
									...
									<thead>
										<tr>
											...
											<th style="max-width: 30px;">Thao tác xóa</th>
										</tr>
									</thead>
									<tbody>
										<c:forEach var="item" items="${allWatch}">
											<tr>
												<td><div 
														style="height: 60px; overflow: hidden; max-width: 55px">${item.watch_id}</div></td>
												<td><div 
														style="height: 60px; overflow: hidden; min-width: 150px">${item.watch_name}</div></td>
												<td><div 
														style="height: 60px; overflow: hidden">${item.price}</div></td>
												<td><div 
														style="height: 60px; overflow: hidden">${item.publisher}</div></td>
												<td><div 
														style="height: 60px; overflow: hidden">${item.year}</div></td>
												<td><form action="/spring-mvc/deleteWatch">
														<input name="watchId" type="hidden"
															value="${item.watch_id}">
														<button>Xóa</button>
													</form></td>
											</tr>
